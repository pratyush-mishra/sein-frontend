...
export async function get_profile(id: number, token: string) :  Promise<UserData> {
   return GET<UserData>(`${process.env.NEXT_PUBLIC_API_URL}/api/users/`+id+"/", token);
}

export async function me_profile(token: string) : Promise<UserData> {
    return GET<UserData>(`${process.env.NEXT_PUBLIC_API_URL}/auth/users/me/`, token);
}

export async function patch_profile(token: string, profileData: ProfileData) : Promise<UserData> {
    return PATCH<UserData>(`${process.env.NEXT_PUBLIC_API_URL}/auth/users/me/`, token, profileData);
}